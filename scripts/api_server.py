...
def speak_text():
    ...
    try:
        ...
        if not _voice:
            return jsonify({
                "success": False,
                "error": "Voice engine not initialized. TTS is disabled."
            }), 503

        # Check if TTS is available
        if not hasattr(_voice, 'tts_engine') or _voice.tts_engine is None:
            return jsonify({
                "success": False,
...
def voice_status():
    ...
    try:
        # Return status even if voice engine failed to initialize
        if not _voice:
            return jsonify({
                "success": True,
                "available": False,
        ...
        return jsonify({
            "success": True,
            "available": True,
            "microphone": getattr(_voice, 'has_microphone', False),
            "tts": _voice.tts_engine is not None if hasattr(_voice, 'tts_engine') else False,
            "is_speaking": getattr(_voice, 'is_speaking', False)
        })
    except Exception as e:
        return jsonify({
            "success": True,
...
if __name__ == '__main__':
    ...
    # Use debug=True but disable reloader to prevent double initialization
