...
export function AiChat({ onCommand }: AiChatProps) {
  ...
  // Minimum time between requests - DISABLED (set to 0 for no limit)
  const MIN_REQUEST_INTERVAL = 0 // No rate limiting
      ...
      if (!response.ok) {
        ...
        if (response.status === 500 && errorData.error) {
          if (errorData.error.includes("API key")) {
            ...
            errorMessage += "**Rate Limit Info**\n\n"
            errorMessage += "The API has rate limits, but they are disabled in this configuration.\n"
            errorMessage += "If you're seeing this, try again or check your API key status."
