  ...
  useEffect(() => {
    const fetchWeather = async () => {
      try {
        ...
        try {
          response = await fetch(`https://wttr.in/${location}?format=j1`, {
            method: "GET",
            headers: {
          ...
          throw new Error("Network error - weather service unavailable")
        }

        clearTimeout(timeoutId)

        if (!response || !response.ok) {
          throw new Error(`Weather API returned ${response?.status || 'unknown'}`)
        }
        ...
        // Suppress console errors for network issues to avoid spam
        ...
        if (error instanceof Error) {
          if (error.name === 'AbortError' || error.message.includes("timed out")) {
            ...
            errorMessage = "Network error - weather service unavailable"
          } else {
